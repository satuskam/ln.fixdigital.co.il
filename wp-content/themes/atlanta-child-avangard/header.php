...
<div id="container" class="<?php echo esc_attr( str_replace( '_', '-', $layout_site ) ); ?>">
				...
				<div class="logo">
					...
					<button type="button" class="navbar-toggle visible-xs" data-toggle="collapse" data-target=".navbar-collapse">
						...
						<span class="icon-bar"></span>
                                        ...
                                        <a class="dialer visible-xs" href="tel:09-7961007" role="link">
	...
	<?php endif; // end blank page ?>
		...
		<?php
			if (is_singular( 'avangard_product' || 'product' )) {
				$product_image_background = get_post_meta( get_the_ID(), 'background_product_image', true );
				$product_image_repeat = get_post_meta( get_the_ID(), 'background_repeat_radio_inline', true );
				$product_background_size = get_post_meta( get_the_ID(), 'background_size_radio_inline', true );

				if ($product_image_background) {
					$product_background_array['image'] = 'background-image: url(' . $product_image_background . ');';
				}
				if ($product_image_repeat) {
					$product_background_array['repeat'] = 'background-repeat: ' . $product_image_repeat . ';';
				}
				if ($product_background_size) {
					$product_background_array['size'] = 'background-size: ' . $product_background_size . ';';
				}
				

				if (!empty($product_background_array)) {
					$product_style = 'style="background-color: transparent; ' . $product_background_array['image'] . $product_background_array['repeat'] . $product_background_array['size'] . ' background-position: center center;"';
				}
				else{
					$product_style = 'background-color: transparent; background-image: url(http://avangard.ln.fixdigital.co.il/wp-content/uploads/sites/909/2018/02/page_mozar.jpg); -webkit-background-size: cover; background-size: cover; background-repeat: repeat-y; background-position: center center;';
				}
		...
		<div id="primary" <?php echo $product_style; ?>>
			<div class="<?php echo WRAP_CLASSES; ?>">
