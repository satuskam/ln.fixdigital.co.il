		...
		<?php else : ?>
			...
			<form action="<?php echo get_option( 'siteurl' ); ?>/wp-comments-post.php" method="post" id="commentform" class="form">
				<?php if ( is_user_logged_in() ) : ?>
					...
					<p class="text-center"><?php printf( __( 'Logged in as <a href="%s/wp-admin/profile.php">%s</a>.', 'pojo' ), get_option( 'siteurl' ), $user_identity ); ?>
						<a href="<?php echo wp_logout_url( get_permalink() ); ?>" title="<?php __( 'Log out of this account', 'pojo' ); ?>"><?php _e( 'Log out &raquo;', 'pojo' ); ?></a>
					</p>

				<?php else : ?>

					<label for="author"><?php _e( 'Name', 'pojo' ); if ( $req ) _e( ' *', 'pojo' ); ?></label>
					<input class="field" type="text" class="text" name="author" id="author" value="<?php echo esc_attr( $comment_author ); ?>" <?php if ( $req ) echo "aria-required='true'"; ?> />

					<label for="email"><?php _e( 'Email', 'pojo' ); if ( $req ) _e( ' *', 'pojo' ); ?></label>
					<input class="field" type="email" class="text" name="email" id="email" value="<?php echo esc_attr( $comment_author_email ); ?>" <?php if ( $req ) echo "aria-required='true'"; ?> />

					<label for="url"><?php _e( 'Website', 'pojo' ); ?></label>
					<input class="field" type="url" class="text" name="url" id="url" value="<?php echo esc_attr( $comment_author_url ); ?>" />

				<?php endif; ?>

				<label for="comment"><?php _e( 'Comment', 'pojo' ); ?></label>
				<textarea id="comment" class="field" name="comment" placeholder="<?php _e( 'Enter your comment', 'pojo' ); ?>" cols="10" rows="10"></textarea>
				<input name="submit" type="submit" id="submit" tabindex="5" value="<?php _e( 'Send Comment', 'pojo' ); ?>" class="button" />
