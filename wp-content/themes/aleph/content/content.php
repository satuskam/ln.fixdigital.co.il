...
$link = false;
?>
<article id="post-<?php the_ID(); ?>" <?php post_class( apply_filters( 'pojo_post_classes', array(), get_post_type() ) ); ?>>
	...
	<div class="entry-post">
		<header class="entry-header">
			<?php if ( ! has_post_format( 'status' ) ) : ?>
				<h2 class="entry-title">
					<?php if ( has_post_format( 'link' ) && ( $link = atmb_get_field( 'format_link_url' ) ) ) : ?>
						<a href="<?php echo esc_url_raw( $link ); ?>"<?php if ( 'blank' === atmb_get_field( 'format_target_link' ) ) echo ' target="_blank"'; ?>><?php the_title(); ?></a>
					<?php else : ?>
		...
		<?php if ( po_archive_metadata_show( 'readmore', $_pojo_parent_id ) ) : ?>
			<p class="add-read-more">
				<?php po_print_archive_readmore( $_pojo_parent_id, $link ); ?>
			</p>
		<?php endif; ?>
	</div>
</article>