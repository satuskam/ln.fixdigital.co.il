...
class IngoApp {
    ...
    public function renderProductCategoryPageContent()
    {
        echo join('', [
            '<div class="categoryProductsContainer container-fluid clearfix">',
                '<div class="sidebar col-md-3 col-sm-12 ">',
                    "<h3 class='col-title accord'> <span class='tit-mark'>+</span>" . __('קטלוג מוצרים', 'ingo') . "</h3>",
                    $this->getCategoriesAccordionMarkup(),
                    $this->addForm(709, 'hidden-sm hidden-xs'), // will be hidden on small screens
                '</div>',

                '<div class="col-md-1 hidden-sm hidden-xs"></div>',

                '<div class="categoryProductsList col-md-8 col-sm-12 clearfix">'
        ]);
        ...
        echo join('', [
                ...
                $this->addForm(709, 'hidden-md hidden-lg'), // will be shown only on small screens
       ...
       foreach ($allCategories as $cat) {
           ...
           /**
            ...
            if( !$this->category_has_children( $cat->cat_ID, 'product_cat' ) ){
                ...
                $accordionHtml .= "[accordion-item class=clearfix title='<span class=\"title\">{$cat->cat_name}</span> <span class=\"plus sign\">+</span><span class=\"minus sign\">&ndash;</span>' state={$state}]";
            }
           ...
           foreach ($allCategories as $subCat) {
               ...
               $accordionHtml .= join('', [
                   '<li>',
                        "<a href='". get_category_link($subCatId) . "' class='{$active}'>",
                            $subCat->cat_name,
                        '</a>',
                    '</li>'
                ]);
