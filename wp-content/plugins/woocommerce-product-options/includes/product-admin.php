...
class Woocommerce_Product_Options_Product_Admin {
    ...
    function save_meta_boxes( $post_id, $post ) {
        ...
        $value = array();
        $post_upload = file_get_contents( "php://input" );
        $pairs = explode( '&', $post_upload );
        $posted_data = array();
    ...
    function add_meta_box( $post ) {
        ...
        print ' />' . __( 'Display product options to Shop and Search Result pages?',
        ...
        print ' />' . __( 'Revert the options back to their default values when the product is added to the cart?',
                        'woocommerce-product-options' ) . '</label><br></td></tr>';
        print '<tr id="accordion-all-options"><td><label><input type="checkbox" value="yes" name="product-options-settings[accordion]" ';
        if ( $accordion == 'yes' ) {
            print 'checked="checked"';
        }
        print ' />' . __( 'Accordion all the options (overrides whether or not each option is accordioned)?',
                        'woocommerce-product-options' ) . '</label><br></td></tr>';
        print '<tr class="accordion-all-options-suboption"><td><label><input type="checkbox" value="yes" name="product-options-settings[accordion_keep_open]" ';
        if ( $accordion_keep_open == 'yes' ) {
            print 'checked="checked"';
        }
        print ' />' . __( 'Allow more than one option to be open in the accordion?',
                        'woocommerce-product-options' ) . '</label><br></td></tr>';
        print '<tr class="accordion-all-options-suboption"><td><label><input type="checkbox" value="yes" name="product-options-settings[accordion_keep_open_on_select]" ';
    ...
    function get_option_label( $type ) {
        if ( $type == 'radio' ) {
            return __( 'radio buttons', 'woocommerce-product-options' );
        } elseif ( $type == 'radio_image' ) {
            return __( 'image options', 'woocommerce-product-options' );
        } elseif ( $type == 'select' ) {
            return __( 'dropdown', 'woocommerce-product-options' );
        } else {
    ...
    function before_input( $id, $meta ) {
        print '<div class="backend-product-option backend-product-option-' . $id . ' backend-product-option-' . value( $meta,
                        'type' ) . '">';
        ?>
        <h2 class="backend-product-option-accordion"><?php print $this->get_option_label( value( $meta, 'type', '' ) ); ?> : <span class="backend-option-title"><?php
        ...
        <div class="backend-product-option-toggle"><div class="backend-outside-preview">
            ...
            function required( $name, $meta ) {
                ?><br class="required-field">
                <label class="required-field" for="<?php print $name; ?>-required">
