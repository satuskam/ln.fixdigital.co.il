...
if ( !empty( $_REQUEST[ 'cancel' ] ) && $_REQUEST[ 'cancel' ] == 'yes' && wp_verify_nonce( value( $_REQUEST,
                        'woocommerce_product_options_image_upload' ),
                'woocommerce_product_options_image_upload_form' ) ) {
    unset( $_SESSION[ 'product_options_' . $post_id . '_' . $option_id ] );
    $value_chosen = '';
    $cancel = true;
}

function product_options_upload_dir() {
    $subdir = wp_create_nonce( 'product_options_wp_unique_filename' );
    return array(
        'path' => dirname( __FILE__ ) . '/../uploads/' . $subdir,
        'url' => plugins_url() . '/woocommerce-product-options/uploads/' . $subdir,
        'subdir' => $subdir,
        'basedir' => dirname( __FILE__ ) . '/../uploads',
        'baseurl' => plugins_url() . '/woocommerce-product-options/uploads',
        'error' => false,
    );
}

if ( !$cancel && isset( $_POST[ 'post-id' ] ) && wp_verify_nonce( value( $_POST,
                        'woocommerce_product_options_image_upload' ),
                'woocommerce_product_options_image_upload_form' ) ) {
    ...
    $file = 'image-upload';
    add_filter( 'upload_dir', 'product_options_upload_dir' );
...
print '/>';
print '<br><a href="' . plugins_url() . '/woocommerce-product-options/includes/image-upload.php" class="cancel-image-upload">' . __( 'Cancel upload',
                'woocommerce-product-options' ) . '</a>';
wp_nonce_field( 'woocommerce_product_options_image_upload_form',
        'woocommerce_product_options_image_upload' );
print '<input type="hidden" name="post-id" value="' . $post_id . '" />' .
        '<input type="hidden" name="option-id" value="' . $option_id . '" /></form>';
wp_enqueue_script( 'jquery' );
wp_enqueue_script( 'woocommerce-product-options-image-upload',
        plugins_url() . '/woocommerce-product-options/assets/js/image-upload.js',
