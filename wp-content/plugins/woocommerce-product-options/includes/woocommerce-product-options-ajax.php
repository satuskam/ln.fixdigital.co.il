...
class WooCommerce_Product_Options_Ajax {
    ...
    function ajax_update_product_options() {
        ...
        if ( !empty( $_POST[ 'product_options' ] ) ) {
            foreach ( $_POST[ 'product_options' ] as $product_options_post_id =>
                        $options ) {
                $product_options = get_post_meta( $product_options_post_id,
                ...
                foreach ( $product_options as $option_id => $product_option ) {
                    ...
                    $this->update_product_option( $post_id, $option_id,
                            $option_value, $product_options_post_id,
                            $variation_id );
                }
            }
        }
     ...
     * ajax - gets the price
     */
    function update_product_option( $post_id = -1 /* The product id */,
            $product_option_id = -1 /* The option id of the product option (1, 2, 3 depending on whether it is the 1st/2nd/3rd option) */,
            $option = '' /* The value of the option */,
            $product_option_post_id = -1 /* Either the product id or the product group post id */,
            $variation_id = -1 /* The selected variation id */,
            $post_data = array() ) {
        if ( empty( $post_data ) ) {
            $post_data = $_POST;
        }
        if ( !empty( $post_data ) && !empty( $post_data[ 'product-options' ] ) ) {
            $post_id = intval( value( $post_data, 'product_post_id', $post_id ) );
            $product_option_post_id = intval( value( $post_data,
                            'product_option_post_id', $product_option_post_id ) );
            $product_option_id = value( $post_data, 'product_option_id',
                    $product_option_id );
            $option = value( $post_data, 'product_option', $option );
            $variation_id = value( $post_data, 'variation_id', $variation_id );
        } else {
        ...
        if ( !empty( $post_data[ 'product-options-hidden' ] ) &&
                !empty( $post_data[ 'product-options-hidden' ][ $product_option_post_id ] )
                && !empty( $post_data[ 'product-options-hidden' ][ $product_option_post_id ][ $product_option_id ] ) ) {
            $_SESSION[ 'hide_product_options_' . $product_option_post_id . '_' . $product_option_id ]
                    = 'yes';
        ...
        if ( !empty( $product ) ) {
            ob_start();
            $price_html = '<span class="amount">' . $this->get_price_html( $price,
                            $product ) . '</span>';
            if ( $product && $product->is_on_sale() ) {
                $sale_price_difference = floatval( $product->get_regular_price() )
                        - floatval( $product->get_sale_price() );
                $price_html = $product->get_price_html_from_to( $price + $sale_price_difference,
                        $price );
            }
            $response = array(
                'price' => $price_html . ob_get_clean(),
                'option_price' => wc_price( $this->get_option_price( $post_id,
                                $product_option_post_id, $product_option_id,
    ...
    function upload_image( $name ) {
        ...
        if ( !empty( $option[ 'multiple' ] ) ) {
            ...
            $_SESSION[ 'product_options_' . $post_id . '_' . $product_option_id ]
                    = $src;
     ...
     */
    function get_option_price( $product_post_id, $post_id, $product_option_id,
            $product_option ) {
        $post_data = $_POST;
        ...
        if ( $type == 'radio' || $type == 'select' ) {
            ...
            if ( !empty( $product_option[ 'options' ] ) && !empty( $product_option[ 'options' ][ $value ] ) ) {
                ...
                if ( !empty( $product_option[ 'options' ] ) && !empty( $product_option[ 'options' ][ $value ][ 'percentage_price' ] ) ) {
                    $option_price = $option_price + $product_price * floatval( $product_option[ 'options' ][ $value ][ 'percentage_price' ] )
                            / 100;
                }
            }
            ...
            if ( !empty( $value ) ) {
                ...
                foreach ( $value as $selected_option ) {
                    ...
                    if ( !empty( $product_option[ 'options' ] ) && !empty( $product_option[ 'options' ][ $selected_option ] )
                            && !empty( $product_option[ 'options' ][ $selected_option ][ 'price' ] ) ) {
                        $option_price = $option_price + floatval( $product_option[ 'options' ][ $selected_option ][ 'price' ] );
                    }
                    if ( !empty( $product_option[ 'options' ] ) && !empty( $product_option[ 'options' ][ $selected_option ] )
                            && !empty( $product_option[ 'options' ][ $selected_option ][ 'percentage_price' ] ) ) {
                        $option_price = $option_price + $product_price * floatval( $product_option[ 'options' ][ $selected_option ][ 'percentage_price' ] )
                                / 100;
                    }
                }
            }
            ...
            for ( $i = 0; $i < strlen( $value ); $i++ ) {
                if ( ctype_lower( $value[ $i ] ) ) {
                    $number_of_lower_case_letters = $number_of_lower_case_letters
                            + 1;
                } elseif ( ctype_upper( $value[ $i ] ) ) {
                    $number_of_upper_case_letters = $number_of_upper_case_letters
                            + 1;
                }
            }
            ...
            $option_price = (floatval( $value ) - floatval( $starting_from )) * floatval( $percentage_price )
                    / 100;
        } elseif ( $type == 'checkbox' ) {
            if ( !empty( $value ) && 'yes' == $value ) {
                $option_price = floatval( value( $product_option,
                                'checked_price', 0 ) );
            } else {
                $option_price = floatval( value( $product_option,
                                'unchecked_price', 0 ) );
            ...
            $default = value( $product_option, 'default' );
            if ( !empty( $value ) && $value != $default ) {
                $option_price = floatval( value( $product_option,
                                'price_for_image' ) );
            }
        }
        if ( !isset( $post_data[ 'hide_product_options_' . $post_id . '_' . $product_option_id . '_price' ] ) ) {
            $_SESSION[ 'product_options_' . $post_id . '_' . $product_option_id . '_price' ]
                    = $option_price;
        }
        return $option_price;
    }
        ...
        if ( $product ) {
            if ( class_exists( 'WOOCS' ) ) {
                ...
                if ( $WOOCS->is_multiple_allowed ) {
                    remove_filter( 'woocommerce_get_price',
                            array( $WOOCS, 'raw_woocommerce_price' ), 9999 );
                }
    ...
    function get_price( $product_post_id, $variation_id = -1 ) {
        $product_options = get_post_meta( $product_post_id,
                'backend-product-options', true );
        $price = $this->get_product_price( $product_post_id, $variation_id );
        if ( !empty( $product_options ) ) {
            foreach ( $product_options as $product_option_id => $product_option ) {
                $option_price = floatval( $this->get_option_price( $product_post_id,
                                $product_post_id, $product_option_id,
                                $product_option ) );
    ...
    function get_rules_price( $product_option_post_id ) {
        $price = 0;
        $settings = get_post_meta( $product_option_post_id,
                'product-options-settings', true );
        $cost_per_checkbox = value( $settings, 'cost_per_checkbox' );
        if ( !empty( $cost_per_checkbox ) ) {
            $minimum_number_of_checkboxes_before_cost = value( $settings,
                    'minimum_number_of_checkboxes_before_cost', 0 );
            $options = get_post_meta( $product_option_post_id,
            ...
            foreach ( $options as $product_option_id => $option ) {
                ...
                if ( !empty( $option[ 'product_percentage_price' ] ) ) {
                    $product_option = $_SESSION[ 'product_options_' . $product_option_post_id . '_' . $product_option_id ];
                    $price = $price * floatval( $product_option ) * floatval( $option[ 'product_percentage_price' ] )
                            / 100;
                }
            }
            if ( $number_of_checkboxes > $minimum_number_of_checkboxes_before_cost ) {
                $price = $price + ($number_of_checkboxes - $minimum_number_of_checkboxes_before_cost)
                        * $cost_per_checkbox;
            }
        }
        return $price;
