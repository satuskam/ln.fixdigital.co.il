...
class Woocommerce_Product_Options_Order_Option_Group {
    ...
    function __construct() {
        ...
        add_filter( 'woocommerce_cart_calculate_fees',
                array( $this, 'woocommerce_cart_calculate_fees' ), 10, 2 );
//Show free fees
        add_filter( 'woocommerce_get_order_item_totals_excl_free_fees',
                array( $this, 'woocommerce_get_order_item_totals_excl_free_fees' ),
                10, 2 );
        add_filter( 'woocommerce_email_order_meta_fields',
                array( $this, 'woocommerce_email_order_meta_fields' ), 10, 3 );
        add_action( 'woocommerce_checkout_order_review',
                array( $this, 'woocommerce_checkout_order_review_5' ), 5 );
    ...
    function woocommerce_email_order_meta_fields( $fields, $sent_to_admin,
            $order ) {
        ...
        /* $meta = value( $_SESSION, 'order_options_order_meta', 0 );
          if ( !empty( $meta ) ) {
          foreach ( $meta as $met ) {
          $fields[] = $met;
          }
    ...
    function print_order_options( $location ) {
        ...
        if ( !empty( $order_option_groups ) ) {
            foreach ( $order_option_groups as $order_option_group ) {
                $meta = get_post_meta( $order_option_group->ID, 'group_options',
                        true );
    ...
    function get_cart_location_options() {
        return array( 'woocommerce_before_cart' => __( 'Before cart',
                    'woocommerce-product-options' ),
            'woocommerce_before_cart_table' => __( 'Before cart table',
                    'woocommerce-product-options' ),
            'woocommerce_after_cart_table' => __( 'After cart table',
                    'woocommerce-product-options' ),
            'woocommerce_after_cart' => __( 'After cart',
                    'woocommerce-product-options' ), );
    }
    ...
    function get_checkout_location_options() {
        return array(
            ...
            'woocommerce_checkout_before_customer_details' => __( 'Before customer details',
                    'woocommerce-product-options' ),
            'woocommerce_checkout_after_customer_details' => __( 'After customer details',
                    'woocommerce-product-options' ),
            'woocommerce_checkout_before_order_review' => __( 'Before order review',
                    'woocommerce-product-options' ),
            'woocommerce_checkout_at_beginning_of_order_review' => __( 'At the beginning of order review',
                    'woocommerce-product-options' ),
            'woocommerce_checkout_after_order_review' => __( 'After order review',
                    'woocommerce-product-options' ),
        );
    }
    ...
    function woocommerce_cart_calculate_fees( $cart ) {
        ...
        //$_SESSION[ 'order_options_order_meta' ] = array();
        $_SESSION[ 'order_option_fees_in_email' ] = array();
        if ( !empty( $post_ids ) ) {
            foreach ( $post_ids as $post_id ) {
                $this->update_session( $post_id );
                $items = array();
                $items = $product_options_in_cart->woocommerce_add_cart_item_data( $items,
                        $post_id );
                $product_options = get_post_meta( $post_id,
                        'backend-product-options', true );
                if ( !empty( $product_options ) && !empty( $items[ '_product_options' ] ) ) {
                    foreach ( $items[ '_product_options' ] as $item_id => $item ) {
                        foreach ( $product_options as $product_option_id =>
                                    $product_option ) {
                            $amount = $woocommerce_product_options_ajax->get_option_price( $post_id,
                                    $post_id, $product_option_id,
                                    $product_option );
                            /* if ( $amount == 0 ) {
                              $_SESSION[ 'order_options_order_meta' ][] = array(
                              'label' => $items[ '_product_options' ][ $item_id ][ 'name' ],
                              'value' => $items[ '_product_options' ][ $item_id ][ 'value' ] );
                              } else { */
                            $name = $items[ '_product_options' ][ $item_id ][ 'name' ] . __( ' - ',
                                            'woocommerce-product-options' ) . $items[ '_product_options' ][ $item_id ][ 'value' ];
    ...
    function update_session( $product_option_post_id ) {
        ...
        global $woocommerce_product_options_ajax;
        $product_options = get_post_meta( $product_option_post_id,
                'backend-product-options', true );
        if ( !empty( $product_options ) ) {
            foreach ( $product_options as $option_id => $option ) {
                ...
                $_SESSION[ 'product_options_' . $product_option_post_id . '_' . $option_id ]
                        = $value_chosen;
                $woocommerce_product_options_ajax->update_product_option( $product_option_post_id,
                        $option_id, $value_chosen, $product_option_post_id );
            }
        }
    }
    ...
    function group_option_price( $product_post_id ) {
        $price = 0;
        $product_options = get_post_meta( $product_group_post_id,
                'backend-product-options', true );
        if ( !empty( $product_options ) ) {
            foreach ( $product_options as $product_option_id => $product_option ) {
                global $woocommerce_product_options_ajax;
                $option_price = $woocommerce_product_options_ajax->get_option_price( $product_post_id,
                        $product_group_post_id, $product_option_id,
                        $product_option );
    ...
    function _print_options( $options, $product_option_post_id, $post_id ) {
        ...
        //Add the options
        global $woocommerce_product_options_product_frontend;
        $woocommerce_product_options_product_frontend->_print_options( $options,
    ...
    function save_meta_boxes( $post_id, $post ) {
        ...
        $woocommerce_product_options_product_admin->save_meta_boxes( $post_id,
                $post );
//Save group options
    ...
    function register_post_type() {
        register_post_type( 'order_option_group',
                array( 'labels' => array(
                'name' => __( 'Order Option Groups',
                        'woocommerce-product-options' ),
                'singular_name' => __( 'Order Option Group',
                        'woocommerce-product-options' ),
                'menu_name' => __( 'Order Option Groups',
                        'woocommerce-product-options' ),
                'add_new' => __( 'Add Order Option Group',
                        'woocommerce-product-options' ),
                'add_new_item' => __( 'Add New Order Option Group',
                        'woocommerce-product-options' ),
                'edit' => __( 'Edit', 'woocommerce-product-options' ),
                'edit_item' => __( 'Edit Order Option Group',
                        'woocommerce-product-options' ),
                'new_item' => __( 'New Order Option Group',
                        'woocommerce-product-options' ),
                'view' => __( 'View Order Option Group',
                        'woocommerce-product-options' ),
                'view_item' => __( 'View Order Option Group',
                        'woocommerce-product-options' ),
                'search_items' => __( 'Search Order Option Groups',
                        'woocommerce-product-options' ),
                'not_found' => __( 'No Order Option Groups found',
                        'woocommerce-product-options' ),
                'not_found_in_trash' => __( 'No Order Option Groups found in trash',
                        'woocommerce-product-options' ),
                'parent' => __( 'Parent Order Option Group',
                        'woocommerce-product-options' )
            ),
            'description' => __( 'This is where you can add/edit your Order Option Groups.',
                    'woocommerce-product-options' ),
            'public' => false,
            'show_ui' => true,
            ...
            'show_in_menu' => current_user_can( 'manage_woocommerce' ) ? 'woocommerce'
                        : true,
            'hierarchical' => false,
    ...
    function add_meta_boxes() {
        global $woocommerce_product_options_product_admin;
        add_meta_box(
                'woocommerce-product-group-options',
                __( 'Group Options', 'woocommerce-product-options' ),
                array( $this, 'add_meta_box' ), 'order_option_group', 'normal',
                'default'
        );
        add_meta_box(
                'woocommerce-order-options',
                __( 'Order Options', 'woocommerce-product-options' ),
                array( $woocommerce_product_options_product_admin, 'add_meta_box' ),
                'order_option_group', 'normal', 'default'
        );
    ...
    function add_meta_box() {
        ...
        wpshowcase_checkboxes( $checkout_location_options, $meta,
                __( 'Where would you like these options to appear on the checkout page?',
                        'woocommerce-product-options' ), 'group_options' );
    ...
    function enqueue_scripts() {
        wp_enqueue_script( 'woocommerce-product-options-product-group',
                plugins_url() . '/woocommerce-product-options/assets/js/product-group.js',
                array( 'jquery' ), false, true );
        $localize_array = array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) );
        wp_localize_script( 'woocommerce-product-group',
                'woocommerce_product_option_group_settings', $localize_array );
        wp_register_style( 'woocommerce-product-group-css',
                plugins_url() . '/woocommerce-product-options/assets/css/product-group.css' );
