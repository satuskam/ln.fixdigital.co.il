 ...
 * Description: Add custom options with prices to your WooCommerce products. Do not delete this plugin if you do not want to lose the images uploaded by your customers. You can download <a href="http://envato.github.io/wp-envato-market/dist/envato-market.zip">this plugin</a> to update Product Options for WooCommerce.
 * Author: WPShowCase
 * Version: 4.136
...
require_once 'wpshowcase/wpshowcase-functions.php';

class WooCommerce_Product_Options {

    //Creates the object
    function __construct() {
        $has_shown_update_message = get_option( 'has_displayed_woocommerce_product_options_update_message',
                false );
        if ( empty( $has_shown_update_message ) ) {
            add_action( 'admin_notices', array( $this, 'plugin_activated_notice' ) );
            update_option( 'has_displayed_woocommerce_product_options_update_message', true );
        }
        add_filter( 'woocommerce_get_settings_pages',
                array( $this, 'woocommerce_get_settings_pages' ) );
        //Ajax for backend
        add_action( 'wp_ajax_get_product_option_preview',
                array( $this, 'ajax_get_product_option_preview' ) );
        //Multilanguage
        load_plugin_textdomain( 'woocommerce-product-options', false,
                dirname( plugin_basename( __FILE__ ) ) . '/languages' );
        // fix scripts/styles for https
        add_action( 'wp_print_scripts',
                array( $this, 'change_http_scripts_to_https' ), 200 );
        add_action( 'wp_print_styles',
                array( $this, 'change_http_styles_to_https' ), 200 );
        add_action( 'wp_ajax_is_upload_image_option_empty',
                array( $this, 'ajax_is_upload_image_option_empty' ) );
        add_action( 'wp_ajax_nopriv_is_upload_image_option_empty',
                array( $this, 'ajax_is_upload_image_option_empty' ) );
    ...
    function plugin_activated_notice() {
        ...
        <div class="notice notice-info is-dismissible">
            <p>Thank you for activating (or updating) Product Options for WooCommerce. Did you know that you can download updates automatically by installing and activating <a href="http://envato.github.io/wp-envato-market/dist/envato-market.zip">envato-market.zip</a>?</p>
        </div>
            <?php
        }
    ...
    function admin_menu() {
        add_submenu_page( 'woocommerce',
                __( 'Product Option Settings', 'woocommerce-product-options' ),
                __( 'Product Option Settings', 'woocommerce-product-options' ),
                'manage_woocommerce', 'woocommerce-product-options-settings',
    ...
    function settings_link( $links ) {
        $settings = '<a href="edit.php?post_type=order_option_group">' . __( 'Order Option Groups',
                        'woocommerce-product-options' ) . '</a>';
        array_unshift( $links, $settings );
        $settings = '<a href="edit.php?post_type=product_option_group">' . __( 'Product Option Groups',
                        'woocommerce-product-options' ) . '</a>';
        array_unshift( $links, $settings );
        $settings = '<a href="admin.php?page=wc-settings&tab=woocommerce-product-options-settings">' . __( 'Product Option Settings',
                        'woocommerce-product-options' ) . '</a>';
        array_unshift( $links, $settings );
        return $links;
    }
    ...
    function ajax_is_upload_image_option_empty() {
        $image_upload_product_options = value( $_POST,
                'image_upload_product_options' );
        $json = array();
        if ( !empty( $image_upload_product_options ) ) {
            foreach ( $image_upload_product_options as $id => $value ) {
                $id = intval( $id );
                $product_options = get_post_meta( $id,
                ...
                foreach ( $value as $i2 => $v2 ) {
                    ...
                    $value = value( $_SESSION,
                            'product_options_' . $id . '_' . $i2 );
                    $src = value( $product_option, 'default' );
                    if ( !empty( $value ) ) {
                        $src_array = wp_get_attachment_image_src( $value,
                                'shop_single' );
                        if ( !empty( $src_array ) ) {
        ...
        if ( !empty( $_SERVER[ 'HTTPS' ] ) && $_SERVER[ 'HTTPS' ] !== 'off' && !file_exists( WP_PLUGIN_URL . '/woocommerce-product-options/js/image-upload.js' ) ) {
            ...
            foreach ( ( array ) $wp_scripts->registered as $script ) {
                if ( stripos( $script->src, 'http://', 0 ) !== false ) {
                    $script->src = str_replace( 'http://', 'https://',
                            $script->src );
                }
            }
        }
    ...
    function frontend_scripts() {
        ...
        wp_enqueue_script( 'jquery-unserialize',
                plugins_url() . '/woocommerce-product-options/assets/js/jquery.unserialize.js',
                array( 'jquery' ), false, true );
        wp_enqueue_script( 'combobox-pro',
                plugins_url() . '/woocommerce-product-options/combobox-pro/js/combobox-pro.js',
                array( 'jquery' ), false, true );
        wp_enqueue_style( 'combox-pro',
                plugins_url() . '/woocommerce-product-options/combobox-pro/css/combobox-pro.css' );
        ...
        wp_enqueue_script( 'spectrum',
                plugins_url() . '/woocommerce-product-options/spectrum-master/spectrum.js',
                array( 'jquery' ), false, true );
        wp_enqueue_style( 'spectrum-css',
                plugins_url() . '/woocommerce-product-options/spectrum-master/spectrum.css' );
        wp_enqueue_script( 'woocommerce-product-options-frontend',
                plugins_url() . '/woocommerce-product-options/assets/js/product-options-frontend.js',
                array( 'combobox-pro', 'jquery', 'jquery-ui-datepicker', 'jquery-ui-tooltip',
        ...
        if ( class_exists( 'WOOCS' ) ) {
            ...
            if ( in_array( $WOOCS->current_currency, $WOOCS->no_cents ) ) {
                ...
                $number_of_decimals = $WOOCS->get_currency_price_num_decimals( $WOOCS->current_currency,
        ...
        $localize_array = array( 'ajaxurl' => admin_url( 'admin-ajax.php' ),
            'numeric_error' => __( 'Please enter a number',
                    'woocommerce-product-options' ),
            'required_error' => __( 'Required field - please enter a value',
                    'woocommerce-product-options' ),
            'number_error' => __( 'Number field - please enter a number',
                    'woocommerce-product-options' ),
            'min_error' => __( 'Please enter a value greater than or equal to ',
                    'woocommerce-product-options' ),
            'max_error' => __( 'Please enter a value less than or equal to ',
                    'woocommerce-product-options' ),
            'checkboxes_max_error' => __( 'You are trying to select too many values',
                    'woocommerce-product-options' ),
            'checkboxes_min_error' => __( 'Please select more values',
                    'woocommerce-product-options' ),
            'currency_symbol' => $currency_symbol,
            'price_format' => $price_format,
            'free' => __( 'Free', 'woocommerce-product-options' ),
            'number_of_decimals' => $number_of_decimals,
            'error_exists' => __( 'There are errors in your options, please correct them before purchasing this product.',
                    'woocommerce-product-options' ),
            'decimal_separator' => $decimal_separator,
            'thousands_separator' => $thousands_separator,
            'currency_multiplier' => $currency_multiplier,
        );
        wp_localize_script( 'woocommerce-product-options-frontend',
                'woocommerce_product_options_settings', $localize_array );
        wp_register_style( 'woocommerce-product-options-frontend-css',
                plugins_url() . '/woocommerce-product-options/assets/css/product-options-frontend.css' );
