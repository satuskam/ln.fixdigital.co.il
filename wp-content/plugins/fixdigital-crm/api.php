...
define('WEB_GROUP_CRM_PLUGIN_NAME', 'Fixdigital CRM');
...
class WP_WebGroupUcoCrmIntergration {
    ...
    public function init()
    {
        ...
        // Init API to get data from a form which sends email by outer mail service.
        // Prepare the gotten data, create lead and send it to CRM (without email sending)
        add_action('wp_ajax_sendLeadToCrmFromDataGottenByAjax', [$this, 'sendLeadToCrmFromDataGottenByAjax'] );
        add_action('wp_ajax_nopriv_sendLeadToCrmFromDataGottenByAjax', [$this, 'sendLeadToCrmFromDataGottenByAjax'] );
    }
...
//    }
    ...
    private function _getPojoFormFieldsData($formData)
    {
        ...
        foreach ($this->_fieldsNamesTypesMap as $fName => $fData) {
            ...
            // process the field with type: file. Try to get link for uploader file from mail body
            if ($fieldType === 'file' && isset($linkFinder)) {
...
//        $this->_leadsLog->onDeactivate();
    ...
    /*
     *  This handler is needed to get data from a form which sent email by outer mail service.
     *  So it shoud not send email. It should process the gotten data to create the lead and send it to CRM
     */
    public function sendLeadToCrmFromDataGottenByAjax()
    {
        $result = ['status' => 'Ok'];
        $formData = $_POST;
        $this->sendFormInfoToCrm($formData);
